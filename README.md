...
LLM은 글자를 그대로 읽지 않고 token 단위로 텍스트를 처리합니다. token 수는 언어, 문장, 모델의 tokenizer에 따라 달라지지만, AI 코딩 도구에서는 짧고 관용적인 영어 작업 지시가 더 간결하게 표현되는 경우가 많습니다.

아래는 `gpt-4o` tokenizer 기준으로 짧은 개발 요청 5개를 직접 비교한 예시입니다. 항상 영어가 더 적다고 보장할 수는 없지만, 이런 식의 짧은 작업 지시에서는 영어 표현이 더 적은 token으로 표현되었습니다.

![Korean vs English prompt token counts](assets/token-comparison.svg)

따라서 처음부터 영어로 요청하는 습관을 들이면, 장기적으로는 한글로 길게 설명하는 것보다 더 토큰 친화적인 작업 흐름을 만들 수 있습니다.
